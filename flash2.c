...
static unsigned char mask_bits[] = {
   ...
   0xff, 0xff, 0xff, 0xff};
	...
	for (n=0;n<4;n++){
			setcolor(15);		
			graphics (objs[n].x,objs[n].y,ship_width,ship_height,ship_bits);
			setcolor(blue);		
	...
	while(n!=27){
		if(nn==0){
			setcolor(15);		
			graphics (objs[cursors].x,objs[cursors].y,ship_width,ship_height,ship_bits);
			setcolor(blue);		
			graphics (objs[cursors].x,objs[cursors].y,mask_width,mask_height,mask_bits);
			nn=1;
			count++;
			if (count>4){
				count=0;
			...
			if (cursors>3)cursors=0;
		}else{
			setcolor(15);		
			invgraphics (objs[cursors].x,objs[cursors].y,mask_width,mask_height,mask_bits);
			setcolor(blue);		
			graphics (objs[cursors].x,objs[cursors].y,mask_width,mask_height,mask_bits);
